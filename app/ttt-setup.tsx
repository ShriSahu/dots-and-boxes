...
export default function TTTSetupScreen() {
  ...
  const [mode, setMode]     = useState<TTTMode>('2player');
  ...
  const startGame = () => {
    ...
    router.push({ pathname: '/ttt', params: { config: JSON.stringify(config) } });
  };
    ...
    <SafeAreaView style={[s.safe, { backgroundColor: theme.bg }]}>
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={[s.card, { backgroundColor: theme.bgCard, borderColor: theme.border }]}>
          ...
          <View style={s.row}>
            {(['2player', 'ai', 'online'] as TTTMode[]).map(m => {
        ...
        {mode !== 'online' && (
          <View style={[s.card, { backgroundColor: theme.bgCard, borderColor: theme.border }]}>
            ...
            <Text style={[s.inputLabel, { color: theme.p1 }]}>Player 1 (X)</Text>
            <TextInput
            ...
            <Text style={[s.inputLabel, { color: mode === 'ai' ? theme.textMuted : theme.p2 }]}>
              {mode === 'ai' ? 'AI Opponent (O)' : 'Player 2 (O)'}
            </Text>
            <TextInput
          ...
          <View style={[s.card, { backgroundColor: theme.bgCard, borderColor: theme.border }]}>
            ...
            <View style={s.row}>
              {DIFFICULTIES.map(d => (
                <TouchableOpacity
                  key={d}
                  style={[s.diffBtn, { borderColor: theme.border }, difficulty === d && { borderColor: theme.p1, backgroundColor: theme.p1Light }]}
                  onPress={() => setDifficulty(d)}
                >
                  <Text style={[s.modeText, { color: difficulty === d ? theme.p1 : theme.text, fontFamily: theme.fontSemiBold }]}>
        ...
        {mode === 'online' && (
          <View style={[s.card, { backgroundColor: theme.p1Light, borderColor: theme.p1 }]}>
            <Text style={{ color: theme.p1, fontFamily: theme.fontRegular, fontSize: 15, lineHeight: 22 }}>
              🌐  You'll set your name in the next screen when creating or joining a room.
            </Text>
          </View>
        )}
...
function makeStyles(theme: any) {
  return StyleSheet.create({
    ...
    startBtn: {
      borderRadius: 10, paddingVertical: 14, paddingHorizontal: 48, marginTop: 8,
