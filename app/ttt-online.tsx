...
import { useOnlineTTT } from '../src/hooks/useOnlineTTT';
...
import type { TTTResult } from '../src/types/ttt.types';

export default function TTTOnlineScreen() {
  ...
  } = useOnlineTTT(roomCode, myUid, isHost, {
  ...
  useEffect(() => {
    if (state.isGameOver && !prevGameOver.current) {
      ...
      if (winner !== 'draw') playSound('win'); else playSound('draw');
      ...
      if (!coinsAwarded.current && myUid) {
        ...
        const outcomeResult: 'win' | 'draw' | 'loss' = winner === myPlayer ? 'win' : winner === 'draw' ? 'draw' : 'loss';
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.bg }]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={[styles.headerTitle, { color: theme.text, fontFamily: theme.fontHandwritten }]}>Ultimate TTT</Text>
      ...
      {result && (
        <Pressable style={[styles.overlay, { backgroundColor: 'rgba(0,0,0,0.55)' }]}>
          <View style={[styles.resultCard, { backgroundColor: theme.bg, borderColor: theme.border }]}>
            <Text style={{ fontSize: 52, lineHeight: 60 }}>{result.winner === 'draw' ? '🤝' : '🎉'}</Text>
