...
import { useTTTEngine } from '../src/hooks/useTTTEngine';
import UltimateBoard from '../src/components/UltimateBoard';
import type { TTTConfig, TTTResult } from '../src/types/ttt.types';
import { getAnonymousUid } from '../src/services/firebase';
import { awardCoins } from '../src/services/coins';

export default function TTTGameScreen() {
  const params = useLocalSearchParams<{ config: string }>();
  const config: TTTConfig = JSON.parse(params.config as string);
  const { theme } = useTheme();
  const { width } = useWindowDimensions();

  const [toast, setToast]   = useState<{ text: string; color: string } | null>(null);
  const [result, setResult] = useState<TTTResult | null>(null);
  ...
  const { state, isAIThinking, playMove, resetGame } = useTTTEngine(config, {
    onMove: (_move, player, boardWon) => {
      playSound(boardWon ? 'chain' : 'click');
      if (boardWon) {
        const name = player === 1 ? config.p1Name : config.p2Name;
        const color = player === 1 ? theme.p1 : theme.p2;
        showToast(`${name} won a board!`, color);
      }
    },
    onTurnSwitch: (next) => {
  ...
  useEffect(() => {
    if (state.isGameOver && !prevGameOver.current) {
      ...
      const winner: TTTResult['winner'] = state.winner === 1 ? 'p1' : state.winner === 2 ? 'p2' : 'draw';
  ...
  const handleCellTap = (board: number, cell: number) => {
  ...
  const handleShare = useCallback(() => {
    ...
    if (result.winner === 'draw') {
      Share.share({ message: `${result.p1Name} and ${result.p2Name} tied in Ultimate Tic-Tac-Toe! 🤝` });
    } else {
      const winner = result.winner === 'p1' ? result.p1Name : result.p2Name;
      Share.share({ message: `${winner} won Ultimate Tic-Tac-Toe! 🎉` });
    }
  }, [result]);
  ...
  return (
    <SafeAreaView style={[s.safe, { backgroundColor: theme.bg }]}>
      <View style={s.header}>
        ...
        <Text style={[s.headerTitle, { color: theme.text, fontFamily: theme.fontHandwritten }]}>Ultimate TTT</Text>
        <View style={{ width: 44 }} />
      </View>
      ...
      <View style={[s.statusWrap, { borderColor: theme.border, backgroundColor: theme.bgCard }]}>
        <Text style={[s.statusHeadline, { color: state.isGameOver ? theme.text : turnColor, fontFamily: theme.fontSemiBold }]}>
          {state.isGameOver
            ? 'Game over'
            : isAIThinking ? `${config.p2Name} is thinking…` : `${turnName}'s turn`}
        </Text>
        <Text style={[s.statusSubline, { color: theme.textMuted, fontFamily: theme.fontRegular }]}>
          {state.activeBoard === null ? 'Free choice — play any open board' : `Must play board ${state.activeBoard + 1}`}
        </Text>
      </View>

      <View style={s.boardWrap}>
        <UltimateBoard state={state} onCellTap={handleCellTap} disabled={boardDisabled} />
      </View>
      ...
      {result && (
        <Pressable style={[s.overlay, { backgroundColor: 'rgba(42,36,24,0.55)' }]}>
          <View style={[s.resultCard, { backgroundColor: theme.bg, borderColor: theme.border, shadowColor: theme.text }]}>
            <Text style={s.resultEmoji}>{result.winner === 'draw' ? '🤝' : '🎉'}</Text>
            <Text style={[s.resultTitle, { color: theme.text, fontFamily: theme.fontHandwritten }]}>
              {result.winner === 'draw' ? "It's a Draw!" : `${result.winner === 'p1' ? result.p1Name : result.p2Name} Wins!`}
            </Text>
...
function makeStyles(theme: any) {
  return StyleSheet.create({
    ...
    resultTitle: { fontSize: 32, fontWeight: '700', textAlign: 'center' },
