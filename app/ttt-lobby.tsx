...
import { createTTTRoom, joinTTTRoom, subscribeToTTTRoom, abandonTTTRoom } from '../src/services/tttRoom';
import type { TTTOnlineRoom } from '../src/types/ttt.types';

type LobbyView = 'menu' | 'waiting' | 'join-input';

export default function TTTLobbyScreen() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const code = await createTTTRoom(myUid, name);
      setRoomCode(code);
      setView('waiting');

      unsubRef.current = subscribeToTTTRoom(code, (room: TTTOnlineRoom) => {
        if (room.status === 'active' && room.guest.uid) {
          unsubRef.current?.();
          router.replace({ pathname: '/ttt-online', params: { roomCode: code, isHost: 'true', myUid } });
        }
  ...
  const handleJoin = async () => {
    ...
    try {
      await ensureUserProfile(myUid, name);
      const { isHost } = await joinTTTRoom(code, myUid, name);
      router.replace({ pathname: '/ttt-online', params: { roomCode: code, isHost: isHost ? 'true' : 'false', myUid } });
    } catch (e: any) {
  ...
  const handleCancelWait = async () => {
    unsubRef.current?.();
    if (roomCode) abandonTTTRoom(roomCode);
    setView('menu');
    setRoomCode('');
  };
    ...
    <SafeAreaView style={[s.safe, { backgroundColor: theme.bg }]}>
      <View style={s.header}>
        ...
        <Text style={[s.title, { color: theme.text }]}>
          <Text style={{ color: theme.p1 }}>TTT</Text>
      ...
      <View style={s.body}>
        ...
        {view === 'waiting' && (
          <View style={s.waitWrap}>
            ...
            <TouchableOpacity style={[s.copyBtn, { borderColor: theme.p1 }]} onPress={() => Share.share({ message: `Join my Ultimate Tic-Tac-Toe game! Code: ${roomCode}` })}>
